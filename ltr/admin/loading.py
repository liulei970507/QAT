...
def load_network(network_dir=None, checkpoint=None, constructor_fun_name=None, constructor_module=None, **kwargs):
    ...
    # Load network
    checkpoint_dict = torch_load_legacy(checkpoint_path) #['epoch', 'actor_type', 'net_type', 'net', 'net_info', 'constructor', 'optimizer', 'stats', 'settings']
    ...
    if 'constructor' in checkpoint_dict and checkpoint_dict['constructor'] is not None:
        ...
        if net_constr.fun_module.startswith('dlframework.'): # ltr.models.tracking.dimpnet
        ...
        net = net_constr.get() # model_constructor
    else:
        raise RuntimeError('No constructor for the given network.')
    # checkpoint_dict_original = torch_load_legacy('/data/liulei/pytracking/ltr/checkpoints/ltr/dimp/super_dimp_rgbt_01_sft_strict_5e-6_baseline_data/DiMPnet_RGBT_ep0010.pth.tar')
    # for keys in checkpoint_dict_original['net'].keys():
    #     #if checkpoint_dict['net'][keys] == checkpoint_dict_original['net'][keys]:
    #     print(checkpoint_dict['net'][keys] == checkpoint_dict_original['net'][keys])
    #     # else:
    #     import pdb
    #     pdb.set_trace()
