...
class NetWrapper:
    ...
    def load_network(self):
        self.net = load_network(self.net_path, **self.net_kwargs) # load model
        if self.use_gpu:
            # import pdb
            # pdb.set_trace()
            self.cuda()
...
class NetWithBackbone(NetWrapper):
    ...
    def extract_backbone(self, im: torch.Tensor):
        ...
        return self.net.extract_backbone_features(im) # ltr/models/tracking/dimpnet.py
    
    def extract_backbone_v(self, im: torch.Tensor):
        """Extract backbone features from the network.
        Expects a float tensor image with pixel range [0, 255]."""
        im = self.preprocess_image(im)
        return self.net.extract_backbone_features_v(im) 
    
    def extract_backbone_i(self, im: torch.Tensor):
        """Extract backbone features from the network.
        Expects a float tensor image with pixel range [0, 255]."""
        im = self.preprocess_image(im)
        return self.net.extract_backbone_features_i(im) 
