...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, run_id: int = None, display_name: str = None):
        ...
        if self.run_id is None:
            ...
            self.segmentation_dir = '{}{}/{}_{}'.format(env.segmentation_path, self.name, self.parameter_name, self.run_id)
        ...
        if os.path.isdir(tracker_module_abspath):
            ...
            self.tracker_class = tracker_module.get_tracker_class() # DiMP
        else:
    ...
    def _init_visdom(self, visdom_info, debug):
        ...
        if debug > 0 and visdom_info.get('use_visdom', True):
            # try:
                self.visdom = Visdom(debug, {'handler': self._visdom_ui_handler, 'win_id_visible': 'Tracking_visible', 'win_id_infrared': 'Tracking_infrared'},
    ...
    def run_sequence(self, args, seq, visualization=None, debug=None, visdom_info=None, multiobj_mode=None):
        ...
        output = self._track_sequence(args, tracker, seq, init_info)
        return output

    def _track_sequence_3branches(self, args, tracker, seq, init_info):
        # Define outputs
        ...
        # print('seq.dataset',seq.dataset)
        # Initialize
        if seq.dataset in ['gtot', 'rgbt234', 'rgbt210', 'lasher', 'lashertestingset']:
        ...
        if tracker.params.visualization and self.visdom is None:
            ...
            self.visualize(image_i, init_info.get('init_bbox'))

        start_time = time.time()
        if seq.dataset in ['gtot', 'rgbt234', 'rgbt210', 'lasher', 'lashertestingset']:
            out = tracker.initialize(args, seq, image_v, image_i, init_info) # Initialize network
        else:
        ...
        init_default = {'target_bbox': init_info.get('init_bbox'),
                        'target_bbox_v': init_info.get('init_bbox'),
                        'target_bbox_i': init_info.get('init_bbox'),
                        'time': time.time() - start_time,
                        'segmentation': init_info.get('init_mask')}
        if args.save_qam_score:
            init_default['qam_score_layer2'] = out['qam_score_layer2']
            init_default['qam_score_layer3'] = out['qam_score_layer3']
        # if args.save_feature_map:
        #     # init_default['features_layer2'] = out['features_layer2']
        #     init_default['features_layer3'] = out['features_layer3']
        ...
        if seq.dataset in ['gtot', 'rgbt234', 'rgbt210', 'lasher', 'lashertestingset']:
            for frame_path_v, frame_path_i in zip(seq.frames_v[1:], seq.frames_i[1:]):
                ...
                info = seq.frame_info(frame_num) # empty {}
                info['previous_output'] = prev_output # last frame's output bbox
                out = tracker.track(image_v, image_i, info) # output target bbox of this frame
                prev_output = OrderedDict(out)
                _store_outputs(out, {'time': time.time() - start_time})
                segmentation = out['segmentation'] if 'segmentation' in out else None
                if self.visdom is not None:
                    tracker.visdom_draw_tracking_visible(image_v, out['target_bbox'], segmentation)
                    tracker.visdom_draw_tracking_infrared(image_i, out['target_bbox'], segmentation)
                elif tracker.params.visualization:
                    self.visualize(image_v, out['target_bbox'], segmentation)
                    self.visualize(image_i, out['target_bbox'], segmentation)
                ## 此部分只在生成伪标签时用到
                计算当前帧结果与GT的IOU
                gt_this_frame = seq.ground_truth_rect[frame_num]
                result_this_frame = out['target_bbox']
                overlap_this_frame = self.overlap_ratio(np.array(gt_this_frame),np.array(result_this_frame))
                if overlap_this_frame<=0.1:
                    # 重新初始化跟踪器
                    print(frame_num, 'reset')
                    print('box before:', out['target_bbox'])
                    
                    # import pdb
                    # pdb.set_trace()
                    init_info = {}
                    frame_num_idx = frame_num
                    while True:
                        if seq.ground_truth_rect[frame_num_idx].tolist()[2:4] == [0.,0.]:
                            frame_num_idx = frame_num_idx-1
                        else:
                            break
                    init_info['init_bbox'] = seq.ground_truth_rect[frame_num_idx]
                    print('box last:', init_info['init_bbox'])
                    out = tracker.initialize(args, seq, image_v, image_i, init_info)
                    prev_output = OrderedDict(out)
                print(frame_num, overlap_this_frame)
        else:
            for frame_path in seq.frames[1:]:
                while True:
                    if not self.pause_mode:
                        ...
                        time.sleep(0.1)
                frame_num += 1
                image= self._read_image(frame_path) # load each frame image
                start_time = time.time()

                info = seq.frame_info(frame_num) # empty {}
                info['previous_output'] = prev_output # last frame's output bbox
                out = tracker.track(image, info) # output target bbox of this frame
                prev_output = OrderedDict(out)
                _store_outputs(out, {'time': time.time() - start_time})

                segmentation = out['segmentation'] if 'segmentation' in out else None

        for key in ['target_bbox', 'target_bbox_v', 'target_bbox_i', 'segmentation', 'qam_score_layer2', 'qam_score_layer3', 'features_layer2', 'features_layer3']:
            if key in output and len(output[key]) <= 1:
                output.pop(key)

        return output

    def _track_sequence(self, args, tracker, seq, init_info):
        output = {'target_bbox': [],
                  'time': [],
                  'segmentation': []}
        ...
        for key in ['target_bbox', 'segmentation', 'qam_score_layer2', 'qam_score_layer3', 'features_layer2', 'features_layer3']:
            if key in output and len(output[key]) <= 1:
                output.pop(key)
    ...
    def get_backbone_feature(self, seq_framev_path, seq_framei_path, bbox):
        params = self.get_parameters()
        # Initialize
        image_v = self._read_image(seq_framev_path)
        image_i = self._read_image(seq_framei_path)
        backbone_feat = self.create_tracker(params).get_backbone_feature(image_v, image_i, bbox) # Initialize network
    ...
    def run_votrgbtir2020(self, debug=None, visdom_info=None):
        print('------run_votrgbtir2020-------')
        params = self.get_parameters()
        params.tracker_name = self.name
        params.param_name = self.parameter_name
        params.run_id = self.run_id
        ...
        import pytracking.evaluation.votrgbtir2020 as vot
        ...
        """Run tracker on VOT."""
        print('------Run tracker on VOT-------')
        if output_segmentation:
            handle = vot.VOT("mask", channels='rgbt')
        else:
            handle = vot.VOT("rectangle", channels='rgbt')
        print('---------handle----------', handle)
        vot_anno = handle.region()
        image_path = handle.frame()
        if not image_path:
