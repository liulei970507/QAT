...
#            self.visdom.register_event_handler(self.block_list_callback_handler, 'featuremap_ui')
        ...
        data = torch.mean(data, dim=0)
        #print('VisFeaturemap_ave1 data.shape', data.shape)
        #data = data.squeeze().flip(0)
        #print('VisFeaturemap_ave2 data.shape', data.shape)
        self.raw_data = data
...
class Visdom:
    def __init__(self, debug=0, ui_info=None, visdom_info=None):
        ...
        if ui_info is not None:
            self.visdom.register_event_handler(ui_info['handler'], ui_info['win_id_visible'])
            self.visdom.register_event_handler(ui_info['handler'], ui_info['win_id_infrared'])
    ...
    def register(self, data, mode, debug_level=0, title='Data', **kwargs):
        if title not in self.registered_blocks.keys():
            ...
            if mode == 'image':
                ...
                self.registered_blocks[title] = VisFeaturemap_ave(self.visdom, show_data, title)
            elif 'backbone_feat' in mode:
                self.registered_blocks[title] = VisFeaturemap_ave(self.visdom, show_data, title)
            else:
