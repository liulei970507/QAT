...
class DiMP(BaseTracker):
    ...
    def initialize_features(self):
        if not getattr(self, 'features_initialized', False):
            self.params.net.initialize() # /home/liulei/pytracking/pytracking/features/net_warppers/load_network load model
            self.params.net_v.initialize()
            self.params.net_i.initialize()
        self.features_initialized = True

    def initialize(self, args, seq, image_v, image_i, info: dict) -> dict:
        ...
        # The DiMP network after load model:pytracking/pytracking/features/net_wrappers
        self.net = self.params.net
        self.net_v = self.params.net_v
        self.net_i = self.params.net_i
        # Time initialization
        tic = time.time()

        # Convert image
        im_v = numpy_to_torch(image_v)
        im_i = numpy_to_torch(image_i)
        ...
        self.target_sz_v = self.target_sz
        self.target_sz_i = self.target_sz
        # Get object id
        ...
        self.target_scale_v =  math.sqrt(search_area) / self.img_sample_sz.prod().sqrt()
        self.target_scale_i =  math.sqrt(search_area) / self.img_sample_sz.prod().sqrt()
        ...
        # init_backbone_feat['layer2'].shape  torch.Size([13, 1024, 44, 44])
        # init_backbone_feat['layer3'].shape  torch.Size([13, 2048, 22, 22])
        backbone_feat_v, backbone_feat_i, init_backbone_feat = self.generate_init_samples(im_v, im_i)
        # Initialize classifier
        self.init_classifier(backbone_feat_v, backbone_feat_i, init_backbone_feat)
 
        # Initialize IoUNet
        if self.params.get('use_iou_net', True):
            self.init_iou_net(backbone_feat_v, backbone_feat_i, init_backbone_feat)
        if args.save_feature_map:
            out = {'time': time.time() - tic, 'features_layer2':torch.cat((backbone_feat_v['layer2'], backbone_feat_i['layer2'])).data.cpu().numpy(), 'features_layer3':torch.cat((backbone_feat_v['layer3'], backbone_feat_i['layer3'])).data.cpu().numpy()}
        else:
            out = {'time': time.time() - tic}
        return out
        
    def track(self, image_v, image_i, info: dict = None) -> dict:
        ...
        backbone_feat_v, sample_coords_v, im_patches_v = self.extract_backbone_features_v(im_v, self.get_centered_sample_pos(),
                                                                      self.target_scale * self.params.scale_factors,
                                                                      self.img_sample_sz)
        backbone_feat_i, sample_coords_i, im_patches_i = self.extract_backbone_features_i(im_i, self.get_centered_sample_pos(),
                                                                      self.target_scale * self.params.scale_factors,
                                                                      self.img_sample_sz)
        if self.args.save_feature_map:
            out['features_layer2'] = torch.cat((backbone_feat_v['layer2'], backbone_feat_i['layer2'])).data.cpu().numpy()
            out['features_layer3'] = torch.cat((backbone_feat_v['layer3'], backbone_feat_i['layer3'])).data.cpu().numpy()
        sample_coords = sample_coords_v # sample_coords_v == sample_coords_i
        backbone_feat = {}
        for key, _ in backbone_feat_v.items():
            backbone_feat[key] = torch.cat((backbone_feat_v[key], backbone_feat_i[key]), dim=1)
        
        backbone_feat_v, _, _ = self.extract_backbone_features_v2(im_v, self.get_centered_sample_pos(),
                                                                      self.target_scale * self.params.scale_factors,
                                                                      self.img_sample_sz)
        backbone_feat_i, _, _ = self.extract_backbone_features_i2(im_i, self.get_centered_sample_pos(),
                                                                      self.target_scale * self.params.scale_factors,
                                                                      self.img_sample_sz)

        # Extract classification features
        test_x = self.get_classification_features(backbone_feat)
        test_x_v = self.get_classification_features_v(backbone_feat_v)
        test_x_i = self.get_classification_features_i(backbone_feat_i)
        # Location of sample
        sample_pos, sample_scales = self.get_sample_location(sample_coords)

        # Compute classification scores
        scores_raw = self.classify_target(test_x)
        scores_raw_v = self.classify_target_v(test_x_v)
        scores_raw_i = self.classify_target_i(test_x_i)
        # Localize the target
        translation_vec, scale_ind, s, flag = self.localize_target(scores_raw, sample_pos, sample_scales)
        translation_vec_v, scale_ind_v, s_v, flag_v = self.localize_target(scores_raw_v, sample_pos, sample_scales)
        translation_vec_i, scale_ind_i, s_i, flag_i = self.localize_target(scores_raw_i, sample_pos, sample_scales)
        new_pos = sample_pos[scale_ind,:] + translation_vec
        new_pos_v = sample_pos[scale_ind_v,:] + translation_vec_v
        new_pos_i = sample_pos[scale_ind_i,:] + translation_vec_i

        # Update position and scale
        if flag_v != 'not_found':
            if self.params.get('use_iou_net', True):
                update_scale_flag = self.params.get('update_scale_when_uncertain', True) or flag_v != 'uncertain'
                if self.params.get('use_classifier', True):
                    self.update_state_v(new_pos_v)
                self.refine_target_box_v(backbone_feat_v, sample_pos[scale_ind_v,:], sample_scales[scale_ind_v], scale_ind_v, update_scale_flag)
            elif self.params.get('use_classifier', True):
                self.update_state_v(new_pos_v, sample_scales[scale_ind_v])
        
        # Update position and scale
        if flag_i != 'not_found':
            if self.params.get('use_iou_net', True):
                update_scale_flag = self.params.get('update_scale_when_uncertain', True) or flag_i != 'uncertain'
                if self.params.get('use_classifier', True):
                    self.update_state_i(new_pos_i)
                self.refine_target_box_i(backbone_feat_i, sample_pos[scale_ind_i,:], sample_scales[scale_ind_i], scale_ind_i, update_scale_flag)
            elif self.params.get('use_classifier', True):
                self.update_state_i(new_pos_i, sample_scales[scale_ind_i])
        ...
        if update_flag and self.params.get('update_classifier', False):
            ...
            if self.visdom is not None:
                self.visdom.register(train_x, 'train_x', 2, 'train_x'+self.id_str)
        ...
        # ------- UPDATE v------- #
        update_flag_v = flag_v not in ['not_found', 'uncertain']
        hard_negative_v = (flag_v == 'hard_negative')
        learning_rate_v = self.params.get('hard_negative_learning_rate', None) if hard_negative_v else None
        if update_flag_v and self.params.get('update_classifier', False):
            train_x_v = test_x_v[scale_ind_v:scale_ind_v+1, ...]
            # Create target_box and label for spatial sample
            target_box_v = self.get_iounet_box(self.pos_v, self.target_sz, sample_pos[scale_ind_v,:], sample_scales[scale_ind_v])
            # Update the classifier model
            self.update_classifier_v(train_x_v, target_box_v, learning_rate_v, s_v[scale_ind_v,...])
        # ------- UPDATE i------- #
        update_flag_i = flag_i not in ['not_found', 'uncertain']
        hard_negative_i = (flag_i == 'hard_negative')
        learning_rate_i = self.params.get('hard_negative_learning_rate', None) if hard_negative_i else None
        if update_flag_i and self.params.get('update_classifier', False):
            train_x_i = test_x_i[scale_ind_i:scale_ind_i+1, ...]
            # Create target_box and label for spatial sample
            target_box_i = self.get_iounet_box(self.pos_i, self.target_sz, sample_pos[scale_ind_i,:], sample_scales[scale_ind_i])
            # Update the classifier model
            self.update_classifier_i(train_x_i, target_box_i, learning_rate_i, s_i[scale_ind_i,...])

        # Set the pos of the tracker to iounet pos
        if self.params.get('use_iou_net', True) and flag != 'not_found' and hasattr(self, 'pos_iounet'):
            self.pos = self.pos_iounet.clone()

        if self.params.get('use_iou_net', True) and flag_v != 'not_found' and hasattr(self, 'pos_iounet'):
            self.pos_v = self.pos_iounet_v.clone()

        if self.params.get('use_iou_net', True) and flag_i != 'not_found' and hasattr(self, 'pos_iounet'):
            self.pos_i = self.pos_iounet_i.clone()
        ...
        if self.visdom is not None:
            ...
            self.visdom.register(backbone_feat_v['layer2'], 'backbone_feat_v layer2', 2, 'backbone_feat_v layer2'+self.id_str)
            self.visdom.register(backbone_feat_i['layer2'], 'backbone_feat_i layer2', 2, 'backbone_feat_i layer2'+self.id_str)
            self.visdom.register(backbone_feat_v['layer3'], 'backbone_feat_v layer3', 2, 'backbone_feat_v layer3'+self.id_str)
            self.visdom.register(backbone_feat_i['layer3'], 'backbone_feat_i layer3', 2, 'backbone_feat_i layer3'+self.id_str)
        elif self.params.debug >= 2:
            show_tensor(score_map, 5, title='Max score = {:.2f}'.format(max_score))

        # Compute output bounding box
        new_state = torch.cat((self.pos[[1,0]] - (self.target_sz[[1,0]]-1)/2, self.target_sz[[1,0]]))
        new_state_v = torch.cat((self.pos_v[[1,0]] - (self.target_sz_v[[1,0]]-1)/2, self.target_sz_v[[1,0]]))
        new_state_i = torch.cat((self.pos_i[[1,0]] - (self.target_sz_i[[1,0]]-1)/2, self.target_sz_i[[1,0]]))
        if self.params.get('output_not_found_box', False) and flag == 'not_found':
            output_state = [-1, -1, -1, -1]
        else:
            output_state = new_state.tolist()

        if self.params.get('output_not_found_box', False) and flag_v == 'not_found':
            output_state_v = [-1, -1, -1, -1]
        else:
            output_state_v = new_state_v.tolist()

        if self.params.get('output_not_found_box', False) and flag_i == 'not_found':
            output_state_i = [-1, -1, -1, -1]
        else:
            output_state_i = new_state_i.tolist()

        # out = {'target_bbox': output_state, 'target_bbox_v': output_state_v, 'target_bbox_i': output_state_i}
        out['target_bbox'] = output_state
        out['target_bbox_v'] = output_state_v
        out['target_bbox_i'] = output_state_i
        return out
    ...
    def classify_target_v(self, sample_x: TensorList):
        """Classify target by applying the DiMP filter."""
        with torch.no_grad():
            scores = self.net_v.classifier.classify(self.target_filter_v, sample_x)
        return scores
    
    def classify_target_i(self, sample_x: TensorList):
        """Classify target by applying the DiMP filter."""
        with torch.no_grad():
            scores = self.net_i.classifier.classify(self.target_filter_i, sample_x)
        return scores
    ...
    def localize_target(self, scores, sample_pos, sample_scales):
        ...
        if preprocess_method == 'none':
            ...
            print('preprocess_method == \'softmax\'')
    ...
    def extract_backbone_features_v(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        with torch.no_grad():
            backbone_feat = self.net.extract_backbone_v(im_patches)#1,3,352,352
        return backbone_feat, patch_coords, im_patches
    
    def extract_backbone_features_i(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        im_patches, patch_coords = sample_patch_multiscale(im, pos, scales, sz,
                                                           mode=self.params.get('border_mode', 'replicate'),
                                                           max_scale_change=self.params.get('patch_max_scale_change', None))
        with torch.no_grad():
            backbone_feat = self.net.extract_backbone_i(im_patches)
        return backbone_feat, patch_coords, im_patches

    def extract_backbone_features_v2(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        im_patches, patch_coords = sample_patch_multiscale(im, pos, scales, sz,
                                                           mode=self.params.get('border_mode', 'replicate'),
                                                           max_scale_change=self.params.get('patch_max_scale_change', None))
        with torch.no_grad():
            backbone_feat = self.net_v.extract_backbone(im_patches)#1,3,352,352
        return backbone_feat, patch_coords, im_patches
    
    def extract_backbone_features_i2(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        im_patches, patch_coords = sample_patch_multiscale(im, pos, scales, sz,
                                                           mode=self.params.get('border_mode', 'replicate'),
                                                           max_scale_change=self.params.get('patch_max_scale_change', None))
        with torch.no_grad():
            backbone_feat = self.net_i.extract_backbone(im_patches)
        return backbone_feat, patch_coords, im_patches
    ...
    def get_classification_features_v(self, backbone_feat):
        with torch.no_grad():
            return self.net_v.extract_classification_feat(backbone_feat)

    def get_classification_features_i(self, backbone_feat):
        with torch.no_grad():
            return self.net_i.extract_classification_feat(backbone_feat)

    def get_iou_backbone_features(self, backbone_feat):
        return self.net.get_backbone_bbreg_feat(backbone_feat)

    def get_iou_backbone_features_v(self, backbone_feat):
        return self.net_v.get_backbone_bbreg_feat(backbone_feat)
    
    def get_iou_backbone_features_i(self, backbone_feat):
        return self.net_i.get_backbone_bbreg_feat(backbone_feat)
    ...
    def get_iou_features_v(self, backbone_feat):
        with torch.no_grad():
            return self.net_v.bb_regressor.get_iou_feat(self.get_iou_backbone_features_v(backbone_feat))

    def get_iou_features_i(self, backbone_feat):
        with torch.no_grad():
            return self.net_i.bb_regressor.get_iou_feat(self.get_iou_backbone_features_i(backbone_feat))
    ...
    def get_iou_modulation_v(self, iou_backbone_feat, target_boxes):
        with torch.no_grad():
            return self.net_v.bb_regressor.get_modulation(iou_backbone_feat, target_boxes)
    
    def get_iou_modulation_i(self, iou_backbone_feat, target_boxes):
        with torch.no_grad():
            return self.net_i.bb_regressor.get_modulation(iou_backbone_feat, target_boxes)
...
#        import pdb
#        pdb.set_trace()
        self.init_sample_pos = self.pos.round()#.int()
        ...
        with torch.no_grad():
            init_backbone_feat_v = self.net.extract_backbone_v(im_patches_v) # ['layer2', 'layer3'] net_wrappers.py
            init_backbone_feat_i = self.net.extract_backbone_i(im_patches_i) # ['layer2', 'layer3'] net_wrappers.py
            init_backbone_feat = {}
            ...
            backbone_feat_v = self.net_v.extract_backbone(im_patches_v) # ['layer2', 'layer3'] net_wrappers.py
            backbone_feat_i = self.net_i.extract_backbone(im_patches_i) # ['layer2', 'layer3'] net_wrappers.py

        return backbone_feat_v, backbone_feat_i, init_backbone_feat
    ...
    def init_target_boxes(self):
        ...
        self.target_boxes_v = self.target_boxes
        self.target_boxes_i = self.target_boxes
        return init_target_boxes

    def init_memory(self, train_x: TensorList, train_x_v: TensorList, train_x_i: TensorList):
        # Initialize first-frame spatial training samples
        self.num_init_samples = train_x.size(0)
        self.num_init_samples_v = train_x_v.size(0)
        self.num_init_samples_i = train_x_i.size(0)
        init_sample_weights = TensorList([x.new_ones(1) / x.shape[0] for x in train_x])
        init_sample_weights_v = TensorList([x.new_ones(1) / x.shape[0] for x in train_x_v])
        init_sample_weights_i = TensorList([x.new_ones(1) / x.shape[0] for x in train_x_i])
        ...
        self.num_stored_samples_v = self.num_init_samples_v.copy()
        self.previous_replace_ind_v = [None] * len(self.num_stored_samples_v)
        self.sample_weights_v = TensorList([x.new_zeros(self.params.sample_memory_size) for x in train_x_v])

        self.num_stored_samples_i = self.num_init_samples_i.copy()
        self.previous_replace_ind_i = [None] * len(self.num_stored_samples_i)
        self.sample_weights_i = TensorList([x.new_zeros(self.params.sample_memory_size) for x in train_x_i])

        for sw, init_sw, num in zip(self.sample_weights, init_sample_weights, self.num_init_samples):
            sw[:num] = init_sw
        for sw, init_sw, num in zip(self.sample_weights_v, init_sample_weights_v, self.num_init_samples_v):
            sw[:num] = init_sw
        for sw, init_sw, num in zip(self.sample_weights_i, init_sample_weights_i, self.num_init_samples_i):
            sw[:num] = init_sw
        # Initialize memory
        ...
        self.training_samples_v = TensorList(
            [x.new_zeros(self.params.sample_memory_size, x.shape[1], x.shape[2], x.shape[3]) for x in train_x_v])
        self.training_samples_i = TensorList(
            [x.new_zeros(self.params.sample_memory_size, x.shape[1], x.shape[2], x.shape[3]) for x in train_x_i])
        for ts, x in zip(self.training_samples, train_x):
            ts[:x.shape[0],...] = x

        for ts, x in zip(self.training_samples_v, train_x_v):
            ts[:x.shape[0],...] = x
        
        for ts, x in zip(self.training_samples_i, train_x_i):
            ts[:x.shape[0],...] = x
    ...
    def update_memory_v(self, sample_x: TensorList, target_box, learning_rate = None):
        # Update weights and get replace ind
        replace_ind = self.update_sample_weights(self.sample_weights_v, self.previous_replace_ind_v, self.num_stored_samples_v, self.num_init_samples_v, learning_rate)
        self.previous_replace_ind_v = replace_ind

        # Update sample and label memory
        for train_samp, x, ind in zip(self.training_samples_v, sample_x, replace_ind):
            train_samp[ind:ind+1,...] = x

        # Update bb memory
        self.target_boxes_v[replace_ind[0],:] = target_box

        self.num_stored_samples_v += 1

    def update_memory_i(self, sample_x: TensorList, target_box, learning_rate = None):
        # Update weights and get replace ind
        replace_ind = self.update_sample_weights(self.sample_weights_i, self.previous_replace_ind_i, self.num_stored_samples_i, self.num_init_samples_i, learning_rate)
        self.previous_replace_ind_i = replace_ind

        # Update sample and label memory
        for train_samp, x, ind in zip(self.training_samples_i, sample_x, replace_ind):
            train_samp[ind:ind+1,...] = x

        # Update bb memory
        self.target_boxes_i[replace_ind[0],:] = target_box

        self.num_stored_samples_i += 1
    ...
    def update_state_v(self, new_pos, new_scale = None):
        # Update scale
        if new_scale is not None:
            self.target_scale_v= new_scale.clamp(self.min_scale_factor, self.max_scale_factor)
            self.target_sz_v = self.base_target_sz * self.target_scale_v

        # Update pos
        inside_ratio = self.params.get('target_inside_ratio', 0.2)
        inside_offset = (inside_ratio - 0.5) * self.target_sz_v
        self.pos_v = torch.max(torch.min(new_pos, self.image_sz - inside_offset), inside_offset)
    
    def update_state_i(self, new_pos, new_scale = None):
        # Update scale
        if new_scale is not None:
            self.target_scale_i = new_scale.clamp(self.min_scale_factor, self.max_scale_factor)
            self.target_sz_i = self.base_target_sz * self.target_scale_i

        # Update pos
        inside_ratio = self.params.get('target_inside_ratio', 0.2)
        inside_offset = (inside_ratio - 0.5) * self.target_sz_i
        self.pos_i = torch.max(torch.min(new_pos, self.image_sz - inside_offset), inside_offset)
    ...
    def init_iou_net(self, backbone_feat_v, backbone_feat_i, backbone_feat):
        # Setup IoU net and objective
        for p in self.net.bb_regressor.parameters():
            p.requires_grad = False

        for p in self.net_v.bb_regressor.parameters():
            p.requires_grad = False
        
        for p in self.net_i.bb_regressor.parameters():
            p.requires_grad = False
        ...
        iou_backbone_feat_v = self.get_iou_backbone_features_v(backbone_feat_v)
        iou_backbone_feat_i = self.get_iou_backbone_features_i(backbone_feat_i)
        # Remove other augmentations such as rotation
        iou_backbone_feat = TensorList([x[:target_boxes.shape[0],...] for x in iou_backbone_feat])
        iou_backbone_feat_v = TensorList([x[:target_boxes.shape[0],...] for x in iou_backbone_feat_v])
        iou_backbone_feat_i = TensorList([x[:target_boxes.shape[0],...] for x in iou_backbone_feat_i])
        # Get modulation vector
        self.iou_modulation = self.get_iou_modulation(iou_backbone_feat, target_boxes)
        self.iou_modulation_v = self.get_iou_modulation_v(iou_backbone_feat_v, target_boxes)
        self.iou_modulation_i = self.get_iou_modulation_i(iou_backbone_feat_i, target_boxes)
        if torch.is_tensor(self.iou_modulation[0]):
            self.iou_modulation = TensorList([x.detach().mean(0) for x in self.iou_modulation])
        if torch.is_tensor(self.iou_modulation_v[0]):
            self.iou_modulation_v = TensorList([x.detach().mean(0) for x in self.iou_modulation_v])
        if torch.is_tensor(self.iou_modulation_i[0]):
            self.iou_modulation_i = TensorList([x.detach().mean(0) for x in self.iou_modulation_i])

    def init_classifier(self, backbone_feat_v, backbone_feat_i, init_backbone_feat):
        # Get classification features
        x = self.get_classification_features(init_backbone_feat)
        x_v = self.get_classification_features_v(backbone_feat_v)
        x_i = self.get_classification_features_i(backbone_feat_i)
        # Overwrite some parameters in the classifier. (These are not generally changed)
        self._overwrite_classifier_params(feature_dim=x.shape[-3], feature_dim_v=x_v.shape[-3], feature_dim_i=x_i.shape[-3])
        ...
        if 'dropout' in self.params.augmentation and self.params.get('use_augmentation', True):
            ...
            x_i = torch.cat([x_i, F.dropout2d(x_i[0:1,...].expand(num,-1,-1,-1), p=prob, training=True)])
        ...
        with torch.no_grad():
            ...
            self.target_filter_v, _, losses_v = self.net_v.classifier.get_filter(x_v, target_boxes, num_iter=num_iter,
                                                                           compute_losses=plot_loss)
            self.target_filter_i, _, losses_i = self.net_i.classifier.get_filter(x_i, target_boxes, num_iter=num_iter,
                                                                           compute_losses=plot_loss)

        # Init memory
        if self.params.get('update_classifier', True):
            self.init_memory(TensorList([x]), TensorList([x_v]), TensorList([x_i]))
    ...
    def _overwrite_classifier_params(self, feature_dim, feature_dim_v, feature_dim_i):
        # Overwrite some parameters in the classifier. (These are not generally changed)
        pred_module = getattr(self.net.classifier.filter_optimizer, 'score_predictor', self.net.classifier.filter_optimizer)
        pred_module_v = getattr(self.net_v.classifier.filter_optimizer, 'score_predictor', self.net_v.classifier.filter_optimizer)
        pred_module_i = getattr(self.net_i.classifier.filter_optimizer, 'score_predictor', self.net_i.classifier.filter_optimizer)
        if self.params.get('label_threshold', None) is not None:
            ...
            self.net_v.classifier.filter_optimizer.label_threshold = self.params.label_threshold
            self.net_i.classifier.filter_optimizer.label_threshold = self.params.label_threshold
        if self.params.get('label_shrink', None) is not None:
            ...
            self.net_v.classifier.filter_optimizer.label_shrink = self.params.label_shrink
            self.net_i.classifier.filter_optimizer.label_shrink = self.params.label_shrink
        if self.params.get('softmax_reg', None) is not None:
            self.net.classifier.filter_optimizer.softmax_reg = self.params.softmax_reg
            self.net_v.classifier.filter_optimizer.softmax_reg = self.params.softmax_reg
            self.net_i.classifier.filter_optimizer.softmax_reg = self.params.softmax_reg
        if self.params.get('filter_reg', None) is not None:
            ...
            pred_module_v.filter_reg[0] = self.params.filter_reg
            pred_module_v.min_filter_reg = self.params.filter_reg
            pred_module_i.filter_reg[0] = self.params.filter_reg
            pred_module_i.min_filter_reg = self.params.filter_reg
        if self.params.get('filter_init_zero', False):
            ...
            self.net_v.classifier.filter_initializer = FilterInitializerZero(self.net_v.classifier.filter_size, feature_dim_v)
            self.net_i.classifier.filter_initializer = FilterInitializerZero(self.net_i.classifier.filter_size, feature_dim_i)
    ...
    def update_classifier_v(self, train_x, target_box, learning_rate=None, scores=None):
        # Set flags and learning rate
        hard_negative_flag = learning_rate is not None
        if learning_rate is None:
            learning_rate = self.params.learning_rate
        ...
        # Update the tracker memory
        if hard_negative_flag or self.frame_num % self.params.get('train_sample_interval', 1) == 0:
            self.update_memory_v(TensorList([train_x]), target_box, learning_rate)
        ...
        if num_iter > 0:
            # Get inputs for the DiMP filter optimizer module
            samples = self.training_samples_v[0][:self.num_stored_samples_v[0],...]
            target_boxes = self.target_boxes_v[:self.num_stored_samples_v[0],:].clone()
            sample_weights = self.sample_weights_v[0][:self.num_stored_samples_v[0]]

            # Run the filter optimizer module
            with torch.no_grad():
                self.target_filter_v, _, losses_v = self.net_v.classifier.filter_optimizer(self.target_filter_v,
                                                                                     num_iter=num_iter, feat=samples,
                                                                                     bb=target_boxes,
                                                                                     sample_weight=sample_weights,
                                                                                     compute_losses=plot_loss)
    
    def update_classifier_i(self, train_x, target_box, learning_rate=None, scores=None):
        # Set flags and learning rate
        hard_negative_flag = learning_rate is not None
        if learning_rate is None:
            learning_rate = self.params.learning_rate
        ...
        # Update the tracker memory
        if hard_negative_flag or self.frame_num % self.params.get('train_sample_interval', 1) == 0:
            self.update_memory_i(TensorList([train_x]), target_box, learning_rate)
        ...
        if num_iter > 0:
            # Get inputs for the DiMP filter optimizer module
            samples = self.training_samples_i[0][:self.num_stored_samples_i[0],...]
            target_boxes = self.target_boxes_i[:self.num_stored_samples_i[0],:].clone()
            sample_weights = self.sample_weights_i[0][:self.num_stored_samples_i[0]]

            # Run the filter optimizer module
            with torch.no_grad():
                self.target_filter_i, _, losses_i = self.net_i.classifier.filter_optimizer(self.target_filter_i,
                                                                                     num_iter=num_iter, feat=samples,
                                                                                     bb=target_boxes,
                                                                                     sample_weight=sample_weights,
                                                                                     compute_losses=plot_loss)
    ...
    def refine_target_box_v(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        """Run the ATOM IoUNet to refine the target bounding box."""

        if hasattr(self.net_v.bb_regressor, 'predict_bb'):
            return self.direct_box_regression_v(backbone_feat, sample_pos, sample_scale, scale_ind, update_scale)

        # Initial box for refinement
        init_box = self.get_iounet_box(self.pos_v, self.target_sz, sample_pos, sample_scale)

        # Extract features from the relevant scale
        iou_features = self.get_iou_features_v(backbone_feat)
        iou_features = TensorList([x[scale_ind:scale_ind+1,...] for x in iou_features])
        ...
        # Optimize the boxes
        output_boxes, output_iou = self.optimize_boxes_v(iou_features, init_boxes)
        ...
        self.pos_iounet_v = new_pos.clone()

        if self.params.get('use_iounet_pos_for_learning', True):
            self.pos_v = new_pos.clone()

        self.target_sz_v = new_target_sz

        if update_scale:
            self.target_scale_v = new_scale

        # self.visualize_iou_pred(iou_features, predicted_box)

    def refine_target_box_i(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        """Run the ATOM IoUNet to refine the target bounding box."""

        if hasattr(self.net_i.bb_regressor, 'predict_bb'):
            return self.direct_box_regression_i(backbone_feat, sample_pos, sample_scale, scale_ind, update_scale)

        # Initial box for refinement
        init_box = self.get_iounet_box(self.pos_i, self.target_sz, sample_pos, sample_scale)

        # Extract features from the relevant scale
        iou_features = self.get_iou_features_i(backbone_feat)
        iou_features = TensorList([x[scale_ind:scale_ind+1,...] for x in iou_features])
        ...
        # Optimize the boxes
        output_boxes, output_iou = self.optimize_boxes_i(iou_features, init_boxes)
        ...
        self.pos_iounet_i = new_pos.clone()

        if self.params.get('use_iounet_pos_for_learning', True):
            self.pos_i = new_pos.clone()

        self.target_sz_i = new_target_sz

        if update_scale:
            self.target_scale_i = new_scale
    ...
    def optimize_boxes_v(self, iou_features, init_boxes):
        box_refinement_space = self.params.get('box_refinement_space', 'default')
        if box_refinement_space == 'default':
            return self.optimize_boxes_default(iou_features, init_boxes)
        if box_refinement_space == 'relative':
            return self.optimize_boxes_relative_v(iou_features, init_boxes)
        raise ValueError('Unknown box_refinement_space {}'.format(box_refinement_space))
    
    def optimize_boxes_i(self, iou_features, init_boxes):
        box_refinement_space = self.params.get('box_refinement_space', 'default')
        if box_refinement_space == 'default':
            return self.optimize_boxes_default(iou_features, init_boxes)
        if box_refinement_space == 'relative':
            return self.optimize_boxes_relative_i(iou_features, init_boxes)
        raise ValueError('Unknown box_refinement_space {}'.format(box_refinement_space))
    ...
    def optimize_boxes_relative_v(self, iou_features, init_boxes):
        """Optimize iounet boxes with the relative parametrization ised in PrDiMP"""
        output_boxes = init_boxes.view(1, -1, 4).to(self.params.device)
        step_length = self.params.box_refinement_step_length
        if isinstance(step_length, (tuple, list)):
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            bb_init = bbutils.rel_to_rect(bb_init_rel, sz_norm)
            outputs = self.net_v.bb_regressor.predict_iou(self.iou_modulation_v, iou_features, bb_init)
    ...
    def optimize_boxes_relative_i(self, iou_features, init_boxes):
        """Optimize iounet boxes with the relative parametrization ised in PrDiMP"""
        output_boxes = init_boxes.view(1, -1, 4).to(self.params.device)
        step_length = self.params.box_refinement_step_length
        if isinstance(step_length, (tuple, list)):
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            bb_init = bbutils.rel_to_rect(bb_init_rel, sz_norm)
            outputs = self.net_i.bb_regressor.predict_iou(self.iou_modulation_i, iou_features, bb_init)
    ...
    def direct_box_regression_v(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        """Implementation of direct bounding box regression."""
        ...
        # Optimize the boxes
        output_boxes = self.net_v.bb_regressor.predict_bb(self.iou_modulation_v, iou_features, init_boxes).view(-1,4).cpu()
    ...
    def direct_box_regression_i(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        """Implementation of direct bounding box regression."""
        ...
        # Optimize the boxes
        output_boxes = self.net_i.bb_regressor.predict_bb(self.iou_modulation_i, iou_features, init_boxes).view(-1,4).cpu()
    ...
    def visdom_draw_tracking_visible(self, image, box, segmentation=None):
        if hasattr(self, 'search_area_box'):
            self.visdom.register((image, box, self.search_area_box), 'Tracking_visible', 1, 'Tracking_visible')
        else:
            self.visdom.register((image, box), 'Tracking_visible', 1, 'Tracking_visible')
    
    def visdom_draw_tracking_infrared(self, image, box, segmentation=None):
        if hasattr(self, 'search_area_box'):
            self.visdom.register((image, box, self.search_area_box), 'Tracking_infrared', 1, 'Tracking_infrared')
        else:
            self.visdom.register((image, box), 'Tracking_infrared', 1, 'Tracking_infrared')
