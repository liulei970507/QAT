...
class DiMP(BaseTracker):
    ...
    def initialize_features(self):
        if not getattr(self, 'features_initialized', False):
            self.params.net.initialize() # /data/liulei/pytracking/pytracking/features/net_warppers/load_network load model
    ...
    def initialize(self, args, seq, image_v, image_i, info: dict) -> dict:
        ...
        # try:
        init_backbone_feat, rgb_guide_t_layer2, rgb_guide_t_layer3 = self.generate_init_samples(im_v, im_i)
        # except:
        # init_backbone_feat = self.generate_init_samples(im_v, im_i)
        ...
        try:
            out = {'time': time.time() - tic, 'qam_score_layer2': rgb_guide_t_layer2.mean(0).data.cpu().numpy(), 'qam_score_layer3': rgb_guide_t_layer3.mean(0).data.cpu().numpy()}
        except:
            out = {'time': time.time() - tic, 'qam_score_layer2': 0., 'qam_score_layer3': 0.}
    ...
    def track(self, image_v, image_i, info: dict = None) -> dict:
        ...
        # add by liulei
        # try:
        #     rgb_guide_t_layer2 = self.net.qa_layer2(torch.cat((backbone_feat_v['layer2'], backbone_feat_i['layer2']),dim=1))
        #     rgb_guide_t_layer3 = self.net.qa_layer3(torch.cat((backbone_feat_v['layer3'], backbone_feat_i['layer3']),dim=1))
        #     backbone_feat_i = self.net.rgb_feature_guide_t_feature(backbone_feat_v, backbone_feat_i, rgb_guide_t_layer2[:,0].unsqueeze(-1), rgb_guide_t_layer3[:,0].unsqueeze(-1))
        #     backbone_feat_v = self.net.t_feature_guide_rgb_feature(backbone_feat_v, backbone_feat_i, rgb_guide_t_layer2[:,1].unsqueeze(-1), rgb_guide_t_layer3[:,1].unsqueeze(-1))
        #     backbone_feat = OrderedDict({key: torch.cat((backbone_feat_v[key],backbone_feat_i[key]), dim=1) for key in backbone_feat_v.keys()})
        # except:
        #     # rgb_guide_t_layer2 = self.net.qa_layer2(torch.cat((backbone_feat_v['layer2'], backbone_feat_i['layer2']),dim=1))
        try:
            rgb_guide_t_layer3 = self.net.qa_layer3(torch.cat((backbone_feat_v['layer3'], backbone_feat_i['layer3']),dim=1))
        except:
            rgb_guide_t_layer3 = [None]*16
            for idx in range(16):
                rgb_guide_t_layer3[idx] = self.net.qa_layer3[idx](torch.cat((backbone_feat_v['layer3'], backbone_feat_i['layer3']),dim=1))
            # import pdb
            # pdb.set_trace()
            rgb_guide_t_layer3 = torch.stack(rgb_guide_t_layer3,1)
            rgb_guide_t_layer3 = rgb_guide_t_layer3.mean(dim=1)
            # print(self.frame_num, rgb_guide_t_layer3)
        # dimp50_rgbt_film_1weight_bn
        backbone_feat_v_ori = backbone_feat_v
        backbone_feat_i_ori = backbone_feat_i
        # try:
        #     backbone_feat_i = self.net.rgb_feature_guide_t_feature(backbone_feat_v, backbone_feat_i, rgb_guide_t_layer3[:,0].unsqueeze(-1))
        #     backbone_feat_v = self.net.t_feature_guide_rgb_feature(backbone_feat_v, backbone_feat_i, rgb_guide_t_layer3[:,1].unsqueeze(-1))
        # except:    
        try:
            backbone_feat_i = self.net.rgb_feature_guide_t_feature(backbone_feat_v_ori, backbone_feat_i_ori, rgb_guide_t_layer3)
            backbone_feat_v = self.net.t_feature_guide_rgb_feature(backbone_feat_v_ori, backbone_feat_i_ori, 1-rgb_guide_t_layer3)
        except: # dimp50_rgbt_film_2weight_soft_16cls_v1
            # print('2 weight')
            backbone_feat_i = self.net.rgb_feature_guide_t_feature(backbone_feat_v_ori, backbone_feat_i_ori, rgb_guide_t_layer3[:,0].unsqueeze(-1))
            backbone_feat_v = self.net.t_feature_guide_rgb_feature(backbone_feat_v_ori, backbone_feat_i_ori, rgb_guide_t_layer3[:,1].unsqueeze(-1))

        backbone_feat = OrderedDict({key: torch.cat((backbone_feat_v[key],backbone_feat_i[key]), dim=1) for key in backbone_feat_v.keys()})
        #     rgb_guide_t_layer2 = rgb_guide_t_layer3
            # pass
        ...
        try:
            out = {'target_bbox': output_state, 'qam_score_layer2': rgb_guide_t_layer2.data.cpu().numpy(), 'qam_score_layer3': rgb_guide_t_layer3.data.cpu().numpy()}
        except:
    ...
    def generate_init_samples(self, im_v: torch.Tensor, im_i: torch.Tensor) -> TensorList:
        ...
        with torch.no_grad():
            ...
            if self.args.first_frame_guide:
                # print('1111')
        #         # try:
        #         #     rgb_guide_t_layer2 = self.net.qa_layer2(torch.cat((init_backbone_feat_v['layer2'], init_backbone_feat_i['layer2']),dim=1))
        #         #     rgb_guide_t_layer3 = self.net.qa_layer3(torch.cat((init_backbone_feat_v['layer3'], init_backbone_feat_i['layer3']),dim=1))
        #         #     init_backbone_feat_i = self.net.rgb_feature_guide_t_feature(init_backbone_feat_v, init_backbone_feat_i, rgb_guide_t_layer2[:,0].unsqueeze(-1), rgb_guide_t_layer3[:,0].unsqueeze(-1))
        #         #     init_backbone_feat_v = self.net.t_feature_guide_rgb_feature(init_backbone_feat_v, init_backbone_feat_i, rgb_guide_t_layer2[:,1].unsqueeze(-1), rgb_guide_t_layer3[:,1].unsqueeze(-1))
                try:
                    # rgb_guide_t_layer2 = self.net.qa_layer2(torch.cat((init_backbone_feat_v['layer2'], init_backbone_feat_i['layer2']),dim=1))
                    rgb_guide_t_layer3 = self.net.qa_layer3(torch.cat((init_backbone_feat_v['layer3'], init_backbone_feat_i['layer3']),dim=1))
                    # dimp50_rgbt_film_1weight_bn
                    ...
                    # dimp50_rgbt_film_1weight_soft_16cls_v1
                    rgb_guide_t_layer3 = [None]*16
                    for idx in range(16):
                        rgb_guide_t_layer3[idx] = self.net.qa_layer3[idx](torch.cat((init_backbone_feat_v['layer3'], init_backbone_feat_i['layer3']),dim=1))
                    rgb_guide_t_layer3 = torch.stack(rgb_guide_t_layer3,1)
                    rgb_guide_t_layer3 = rgb_guide_t_layer3.mean(dim=1)
                    ...
                    # pdb.set_trace()
                rgb_guide_t_layer2 = rgb_guide_t_layer3
                init_backbone_feat_v_ori = init_backbone_feat_v
                init_backbone_feat_i_ori = init_backbone_feat_i
                # try:
                #     init_backbone_feat_i = self.net.rgb_feature_guide_t_feature(init_backbone_feat_v, init_backbone_feat_i, rgb_guide_t_layer3[:,0].unsqueeze(-1))
                #     init_backbone_feat_v = self.net.t_feature_guide_rgb_feature(init_backbone_feat_v, init_backbone_feat_i, rgb_guide_t_layer3[:,1].unsqueeze(-1))
                # except:
                try: # dimp50_rgbt_film_1weight_soft_16cls_v1
                    init_backbone_feat_i = self.net.rgb_feature_guide_t_feature(init_backbone_feat_v_ori, init_backbone_feat_i_ori, rgb_guide_t_layer3)
                    init_backbone_feat_v = self.net.t_feature_guide_rgb_feature(init_backbone_feat_v_ori, init_backbone_feat_i_ori, 1-rgb_guide_t_layer3)
                except: # dimp50_rgbt_film_2weight_soft_16cls_v1
                    # print('init 2 weight')
                    init_backbone_feat_i = self.net.rgb_feature_guide_t_feature(init_backbone_feat_v_ori, init_backbone_feat_i_ori, rgb_guide_t_layer3[:,0].unsqueeze(-1))
                    init_backbone_feat_v = self.net.t_feature_guide_rgb_feature(init_backbone_feat_v_ori, init_backbone_feat_i_ori, rgb_guide_t_layer3[:,1].unsqueeze(-1))
            else:
                rgb_guide_t_layer2 = rgb_guide_t_layer3 = None
            init_backbone_feat = OrderedDict({key: torch.cat((init_backbone_feat_v[key],init_backbone_feat_i[key]), dim=1) for key in init_backbone_feat_v.keys()})
        # try:
        return init_backbone_feat, rgb_guide_t_layer2, rgb_guide_t_layer3
        # except:
